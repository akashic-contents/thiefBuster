...
var ResultSubscene = /** @class */ (function (_super) {
    ...
    ResultSubscene.prototype.onRollEnd = function () {
        ...
        this.isRolling = false;
        this.setScoreLabelText();
        if (gameUtil_1.gameUtil.isRankingMode()) {
            // ランキングモードであれば、スコアを引数に終了する
            this.scene.setTimeout(3000, function () {
                // console.log("onRollEnd: setTimeout: scoreValue:"+this.scoreValue+".");
                g.game.external.bidirection.currentSession.quit(_this.scoreValue);
            });
        }
        else if (commonDefine_1.commonDefine.ENABLE_RETRY) {
            // リトライ操作を受け付ける場合
    ...
    ResultSubscene.prototype.createTips = function () {
        ...
        var spr = new g.Sprite({ scene: this.scene, src: this.scene.assets[asset], width: size.width, height: size.height });
